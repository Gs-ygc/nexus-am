...
int main() {
    ...
    // Use aligned_alloc which is C11 standard and supported by musl/glibc
    A = (int8_t *)malloc(M * K * sizeof(int8_t));
    B = (int8_t *)malloc(N * K * sizeof(int8_t));
    C_ame = (int32_t *)malloc(M * N * sizeof(int32_t));
    ...
    if (total_failures == 0) {
        printf("\nAll %d fuzzing tests PASSED.\n", num_tests);
         ...
         free(A); free(B); free(C_ame); free(C_ref);
         return 1;
    }
